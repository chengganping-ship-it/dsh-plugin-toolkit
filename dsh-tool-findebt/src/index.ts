...
interface TargetCompany {
  ...
  headquarters: string;
  subsidiaries: string[];
  key_executives: Array<{ name: string; title: string; tenure_years: number }>;
  ownership_structure: Array<{ type: string; percentage: number; holder: string }>;
  business_description: string;
  revenue_streams: Array<{ name: string; percentage: number }>;
  key_products: string[];
  primary_markets: string[];
  major_customers: Array<{ name: string; revenue_percentage: number }>;
  major_suppliers: Array<{ name: string; dependency_level: string }>;
}
...
interface CashFlowStatement {
  period: string;
  operating_activities: { net_income: number; depreciation: number; amortization: number; working_capital_change: number; other_non_cash: number; total: number };
  investing_activities: { capex: number; acquisitions: number; asset_sales: number; investments: number; total: number };
  financing_activities: { debt_proceeds: number; debt_repayments: number; equity_issuance: number; dividends: number; share_repurchases: number; total: number };
...
interface ToolResult {
  ...
  tool_name: string;
  output: string;
  metadata: { generated_at: string; processing_time_ms: number; confidence_level: string; disclaimers_applied: string[] };
}
...
interface ValuationSensitivity {
  variable: string;
  base_case: number;
  bull_case: number;
  bear_case: number;
  ev_impact_base: number;
  ev_impact_bull: number;
  ev_impact_bear: number;
}

interface FraudIndicatorDetail {
...
function fmtCur(v: number, u: string = ''): string {
  if (v === 0) return '$0' + u;
  if (Math.abs(v) >= 1e9) return '$' + (v / 1e9).toFixed(1) + 'B' + u;
  if (Math.abs(v) >= 1e6) return '$' + (v / 1e6).toFixed(1) + 'M' + u;
  if (Math.abs(v) >= 1e3) return '$' + (v / 1e3).toFixed(0) + 'K' + u;
...
function rt(v: number, d: number): number {
  const f = Math.pow(10, d);
  return Math.round(v * f) / f;
}

function sc(s: number): string {
  if (s >= 7) return ':green_circle:';
  if (s >= 4) return ':yellow_circle:';
  return ':red_circle:';
}

function sevE(sev: string): string {
  if (sev === 'critical') return ':red_circle:';
  if (sev === 'high') return ':orange_circle:';
  if (sev === 'medium') return ':yellow_circle:';
  return ':green_circle:';
}
...
function assessStrategicFit(target: TargetCompany, dealType: string): { market_position: { assessment: string; score: number }; synergy: { assessment: string; score: number }; integration: { assessment: string; score: number }; culture: { assessment: string; score: number }; overall: number } {
...
function generateDealRisks(target: TargetCompany, dealType: string): Array<{ category: string; level: string; description: string; mitigation: string }> {
  return [
...
function buildRiskMatrix(entries: RiskMatrixEntry[]): { entries: RiskMatrixEntry[]; maxScore: number; avgScore: number; criticalCount: number; highCount: number; moderateCount: number; lowCount: number } {
  let maxScore = 0;
  let totalScore = 0;
  let criticalCount = 0;
  let highCount = 0;
  ...
  entries.forEach(function (e) {
    ...
    if (e.risk_score >= 15) criticalCount++;
    else if (e.risk_score >= 10) highCount++;
    else if (e.risk_score >= 5) moderateCount++;
    else lowCount++;
    if (e.risk_score > maxScore) maxScore = e.risk_score;
  });
...
function buildComplianceFindings(records: CompanyRecord[]): ComplianceFinding[] {
...
function generateValuationSensitivity(ev: number, fcf: number, wacc: number): ValuationSensitivity[] {
  return [
...
function detectAdvancedFraudPatterns(transactions: TransactionRecord[]): FraudIndicatorDetail[] {
  const indicators: FraudIndicatorDetail[] = [];
  const totalTx = transactions.length;
  if (totalTx === 0) return indicators;
...
function assessCFLQualityDetailed(statements: CashFlowStatement[]): { metrics: CFQualityMetric[]; overallScore: number; trendDirection: string; recommendation: string } {
  const metrics: CFQualityMetric[] = [];
...
function generateTermSheet(params: DealParameters, tax: TaxConsideration[]): DealTermSheet[] {
  return [
...
function deal_analyst(target_company: TargetCompany, deal_type: string, financial_statements: FinancialStatement[]): ToolResult {
...
function financial_risk_scorer(financial_data: FinancialStatement[], industry_benchmarks: IndustryBenchmark[], macro_factors: MacroFactor[]): ToolResult {
  const startTime = Date.now();
  const output: string[] = [];
...
function compliance_auditor(company_records: CompanyRecord[], regulatory_scope: string[]): ToolResult {
  const startTime = Date.now();
  const output: string[] = [];
...
function valuation_modeler(company_financials: FinancialStatement[], comparable_transactions: ComparableTransaction[], methodology: string): ToolResult {
  const startTime = Date.now();
  const output: string[] = [];
  ...
  let pvExp = 0;
  for (let y = 1; y <= 5; y++) { pvExp += fcf * Math.pow(1.08, y) / Math.pow(1.09, y); }
...
function forensic_accountant(transaction_data: TransactionRecord[], investigation_scope: string[]): ToolResult {
  const startTime = Date.now();
  const output: string[] = [];
...
function cash_flow_analyzer(cash_flow_statements: CashFlowStatement[], projections: Projection[]): ToolResult {
  const startTime = Date.now();
  const output: string[] = [];
...
function red_flag_detector(due_diligence_data: DueDiligenceData[], risk_indicators: RiskIndicator[]): ToolResult {
  const startTime = Date.now();
  const output: string[] = [];
  ...
  output.push('|----------|-------|----------|------|--------|-----|');
  const cats: Record<string, { name: string; total: number; critical: number; high: number; medium: number; low: number }> = {};
  due_diligence_data.forEach(function (d) {
    if (!cats[d.category]) cats[d.category] = { name: d.category, total: 0, critical: 0, high: 0, medium: 0, low: 0 };
    cats[d.category].total++;
    if (d.materiality === 'highly_material') cats[d.category].critical++;
    else if (d.materiality === 'material') cats[d.category].high++;
    else if (d.materiality === 'moderate') cats[d.category].medium++;
    else cats[d.category].low++;
  });
...
function deal_structurer(deal_parameters: DealParameters, tax_considerations: TaxConsideration[], regulatory_constraints: RegulatoryConstraint[]): ToolResult {
  const startTime = Date.now();
  const output: string[] = [];
